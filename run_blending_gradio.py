...
def create_mask_from_points(points, img_h, img_w):
    ...
    mask_img = Image.new("L", (img_w, img_h), 0)  # 创建空白图像，背景为0
    draw = ImageDraw.Draw(mask_img)
    
    # 使用polygon函数填充多边形区域，填充值为255
    draw.polygon(points.tolist(), outline=255, fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    """
    计算前景和混合图像在掩模区域内的拉普拉斯损失。

    Args:
        foreground_img (torch.Tensor): 前景图像张量，形状为 (1, C, H, W)。
        foreground_mask (torch.Tensor): 前景掩模张量，形状为 (1, 1, H, W)。
        blended_img (torch.Tensor): 混合图像张量，形状为 (1, C, H, W)。
        background_mask (torch.Tensor): 背景掩模张量，形状为 (1, 1, H, W)。

    Returns:
        torch.Tensor: 计算出的拉普拉斯损失。
    """
    # 定义拉普拉斯卷积核
    ...
    # 计算前景和混合图像的拉普拉斯结果
    laplacian_foreground = F.conv2d(foreground_img, laplacian_kernel, padding=1)
    laplacian_blended = F.conv2d(blended_img, laplacian_kernel, padding=1)

    # 仅在掩模区域内计算损失
    mask = (foreground_mask * background_mask).expand_as(laplacian_foreground)
    loss = F.mse_loss(laplacian_foreground * mask, laplacian_blended * mask)
